            ...
            return (
              ...
              >
                <div className="p-4 flex flex-col flex-1">
                  ...
                  <p className="text-sm font-medium text-[#003366] mb-4 line-clamp-3">{req.postSaleItems.map(item => item.productName).join(', ')}</p>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    ...
                    <div className="text-right">
                      <p className="text-[10px] font-medium text-gray-500 uppercase tracking-wide mb-0.5">{req.type.toLowerCase() === 'return' ? 'Số tiền khấu trừ' : 'Số tiền hoàn trả'}</p>
                      <p className="text-base font-bold text-[#003366] tabular-nums">{formatMoney(req.refundAmount ?? 0)}</p>
                    </div>
                  </div>

                  <div className="mb-4">
                    <span className="text-xs text-gray-500">{formatDate(req.requestedTime)}</span>
                  </div>
