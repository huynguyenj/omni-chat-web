...
export default function ClaimInfoSection({ onRefresh }: ClaimInfoSectionProps) {
  ...
  const { control, handleSubmit, register, onSubmit, errors, loading, setConversationId, conversationId, isChangeTaskSelected, setIsChangeTaskSelected, messageError } = useCreateClaim({ onRefresh })
  ...
  return (
    <div>
      ...
      <AnimatePresence>
        {isOpen &&
                    <PopupBasic title='Tạo claim mới' onClose={handleOpen} size='lg'>
                      <div className='sm:w-140'>
                        ...
                        { loadingConversation ?
                          ...
                          <ScrollArea className='h-50 px-3'>
                            ...
                            {messageError && <p className='text-sm-body-desktop text-red-400 font-medium'>{messageError}</p>}
                            <PaginationBar
