...
function OrderButton() {
  ...
  const handleSelectTypeProduct = (type: typeof typeProduct) => {
    if (type === typeProduct) setTypeProduct(undefined)
    setTypeProduct(type)
  }
  const handleSelectCapacity = (capacity: typeof capacityProduct) => {
    if (capacity === capacityProduct) setCapacityProduct(undefined)
  ...
  const filterProductList = useMemo(() => {
    return listProducts.filter((product) => {
      if (capacityProduct && product.capacity.includes(capacityProduct.capacity) || typeProduct && product.category === typeProduct.type)
        return product
    })
          ...
          <PopupBasic title='Tạo đơn hàng mới' onClose={handleOpen}>
            ...
            <div id='index#1'>
              <div className='mt-7'>
                ...
                <div className='flex justify-between items-center'>
                  <p className='text-sm-body-desktop font-medium text-secondary'>Sản phẩm phù hợp</p>
                  <p className='text-sm-body-desktop text-soft-gray bg-gray-200'>{filterProductList.length} sản phẩm</p>
                </div>
                <ScrollArea className='h-30'>
                  { filterProductList.map((product) => (
                    <Card key={product.productId} className='border-2 border-border-primary px-3 py-2'>
                      <div>
                        <p>{product.name}</p>
                        <Tag className={`px-2 py-1 border ${product.category === 'sour' ? 'border border-[#C27AFF] text-[#9810FA]'
