                  ...
                  { filterProductList.map((product) => (
                    <Card key={product.productId} className='border-2 border-border-primary px-3 py-2'>
                      ...
                      {product.capacity.map((cap) => (
                        <Tag key={cap} className='bg-gray-200 text-soft-gray'>
                          {cap}
                        </Tag>
                      ))}
                      <div className='flex'>
                        <p>{product.price.toLocaleString('vi-VN')}</p>
                        <p className='text-sm-body-desktop text-soft-gray font-medium'>Tồn kho: <span className='text-primary ml-3'>{product.storage}</span></p>
                      </div>
                    </Card>
                  )) }
                </ScrollArea>
